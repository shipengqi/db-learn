...
**子查询总是从内向外处理。在处理上面的`SELECT`语句时，MySQL实际上执行了两个操作**。
...
#### 为什么使用联结表
正如所述，分解数据为多个表能更有效地存储，更方便地处理，并且具有更大的可伸缩性。但这些好处是有代价的。如果数据存储在多个表中，怎样用单条`SELECT`语句检索出数据？
...
> **SQL规范首选`INNER JOIN`语法。此外，尽管使用WHERE子句定义联结的确比较简单，但是使用明确的联结语法能够确保不会忘记联结条件，有时候这样做也能影响性能。
