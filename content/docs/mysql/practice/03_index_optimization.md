...
所以查询效率：`count(*)≈count(1)>count(主键 id)>=count(字段)`，`count(字段)` 和 `count(主键 id)` 还需要取出字段判断是否为 `NULL`，所以效率会比 `count(*)` 和 `count(1)` 低。如果 `count(字段)` 有索引，并且是占用存储空间最小的那个索引，那么效率会和 `count(主键 id)` 差不多。
