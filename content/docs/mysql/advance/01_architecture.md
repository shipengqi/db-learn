...
开始执行的时候，要先判断一下你对这个表 T 有没有执行查询的**权限**，如果没有，就会返回没有权限的错误（如果命中查询缓存，会在查询缓存返回结果的时候，做权限验证）。
...
2. 存储引擎在二级索引 `idx_name` 中定位 `name < 's孙权'` 的第一条记录的 `name` 列的值为 `c曹操`。然后拿着该二级索引记录中的主键值去回表，把完整的用户记录都取到之后返回给 Server 层。
...
索引下推是一种在存储引擎层提前过滤不符合条件的记录的优化手段。

没有 ICP： 索引只用来定位数据行的位置，具体 WHERE 条件由 MySQL Server 层来判断。即使部分索引已能判断，仍需回表取数据。

有 ICP： InnoDB 在扫描索引时，就会尽量利用 WHERE 子句中的条件直接过滤数据，不必回表就能丢掉无效行。


ICP 的原理（简化流程）：
优化器分析 WHERE 子句，拆分出可用于索引列的条件（称为 index condition）

若启用了 ICP：

索引扫描时将这些条件下推到存储引擎层，边扫描边过滤

避免无效回表（只有真正可能满足的行才回表）


ICP 的使用条件
查询使用 覆盖不到所有列的索引（非覆盖索引）

WHERE 条件中包含非最左前缀的索引列

存储引擎支持（InnoDB、MyISAM 支持）
