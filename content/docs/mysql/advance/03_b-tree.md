...
哈希表是一种以键-值（key-value）存储数据的结构，插入和查询都很快，但是适用于只有等值查询的场景，**由于 key 是无序的，所以范围查询很慢**。也**不支持模糊查询**。
...
联合索引，本质上也是一个二级索引。例如一个联合索引包含列 `a` 和 `b`：
...
删除数据的速度和创建的索引数量是成正比的。百万级别的数据删除速度也会很慢。最快的方案是**删除重建索引**。
