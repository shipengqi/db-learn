...
分桶平均的基本原理是**将统计数据划分为 `m` 个桶，每个桶分别统计各自的 `k_max`**,​ 并能得到各自的基数预估值，最终**对这些基数预估值求平均**得到整体的基数估计值。LLC 中使用几何平均数预估整体的基数值，但是当统计数据量较小时误差较大；HLL 在 LLC 基础上做了改进，**采用调和平均数**过滤掉不健康的统计值。
